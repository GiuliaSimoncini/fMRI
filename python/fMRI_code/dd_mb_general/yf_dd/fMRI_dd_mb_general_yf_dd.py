...
def fMRI_dd_mb_general_yf_dd(subject_id):
    ...
    reach_outputs = linalg.orth(linalg.pinv(U @ (np.eye(100) - K_dd @ (linalg.pinv(L @ K_dd)) @ L) @ linalg.pinv(Yf)))
    n_reach = nr
    ...
    for j in range(n_reach):
        yf_dd = reach_outputs[:, j].reshape(-1,1)  # Ensure column vector

        # Model-based control
        u_mb = ((np.eye(600) - K_mb @ linalg.pinv(M @ K_mb) @ M) @ linalg.pinv(Co)) @ yf_dd
        # Data-driven control
        u_dd = (U @ (np.eye(100) - K_dd @ (linalg.pinv(L @ K_dd)) @ L) @ linalg.pinv(Yf)) @ yf_dd

        y_mb = Co @ u_mb
        y_dd = Co @ u_dd

        # Normalized error model-based
        err_mb[j] = linalg.norm(yf_dd - y_mb) / p
        # Normalized error data-driven
        err_dd[j] = linalg.norm(yf_dd - y_dd) / p
