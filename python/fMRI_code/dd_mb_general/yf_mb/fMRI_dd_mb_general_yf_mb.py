...
def fMRI_dd_mb_general_yf_mb(subject_id):
    ...
    reach_outputs = linalg.orth(linalg.pinv((np.eye(600) - K_mb @ linalg.pinv(M @ K_mb) @ M) @ linalg.pinv(Co)))
    n_reach = nr
    ...
    for j in range(n_reach):
        yf_mb = reach_outputs[:, j].reshape(-1,1)  # Ensure column vector

        # Model-based control
        u_mb = ((np.eye(600) - K_mb @ linalg.pinv(M @ K_mb) @ M) @ linalg.pinv(Co)) @ yf_mb
        # Data-driven control
        u_dd = (U @ (np.eye(100) - K_dd @ (linalg.pinv(L @ K_dd)) @ L) @ linalg.pinv(Yf)) @ yf_mb

        y_mb = Co @ u_mb
        y_dd = Co @ u_dd

        # Normalized error model-based
        err_mb[j] = linalg.norm(yf_mb - y_mb) / p
        # Normalized error data-driven
        err_dd[j] = linalg.norm(yf_mb - y_dd) / p
