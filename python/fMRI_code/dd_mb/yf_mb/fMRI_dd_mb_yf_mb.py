...
def fMRI_dd_mb_yf_mb(subject_id):
    ...
    reach_outputs = linalg.orth(linalg.pinv(Co.T @ linalg.pinv(Co @ Co.T)))
    n_reach = nr
    ...
    for j in range(n_reach):
        ...
        # Model-based control
        u_mb = Co.T @ linalg.pinv(Co @ Co.T) @ yf_mb
        # Data-driven control
        u_dd = linalg.pinv(Yf @ linalg.pinv(U)) @ yf_mb
