...
def fMRI_dd_train_validation_yf_dd(U_train, Yf_train, subject_id_validation):
    ...
    reach_outputs = linalg.orth(Yf @ linalg.pinv(U))
    n_reach = nr
    ...
    for j in range(n_reach):
        yf_dd = reach_outputs[:, j].reshape(-1, 1)

        # Data-driven control
        u_dd = linalg.pinv(Yf @ linalg.pinv(U)) @ yf_dd
        # Data-driven train-validation control
        u_dd_train = linalg.pinv(Yf_train @ linalg.pinv(U_train)) @ yf_dd
        ...
        err_dd_train[j] = linalg.norm(yf_dd - y_dd_train) / p
