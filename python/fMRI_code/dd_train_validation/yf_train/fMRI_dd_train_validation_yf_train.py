...
def fMRI_dd_train_validation_yf_train(U_train, Yf_train, subject_id_validation):
    ...
    reach_outputs = linalg.orth(Yf_train @ linalg.pinv(U_train))
    n_reach = nr
    ...
    for j in range(n_reach):
        yf_train = reach_outputs[:, j].reshape(-1, 1)  # Ensure column vector

        # Data-driven control
        u_dd = linalg.pinv(Yf @ linalg.pinv(U)) @ yf_train
        # Data-driven train-validation control
        u_dd_train = linalg.pinv(Yf_train @ linalg.pinv(U_train)) @ yf_train

        y_dd = Co @ u_dd
        y_dd_train = Co @ u_dd_train

        # Normalized error data-driven
        err_dd[j] = linalg.norm(yf_train - y_dd) / p
        # Normalized error data-driven train-validation
        err_dd_train[j] = linalg.norm(yf_train - y_dd_train) / p
